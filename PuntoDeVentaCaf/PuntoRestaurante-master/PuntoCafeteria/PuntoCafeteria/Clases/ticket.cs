...
namespace PuntoCafeteria.Clases
{
     class ticket
    {
        ...
        private string Fechaventa { set; get; }
        private double Totalticket { set; get; }
        private double Pago { set; get; }
        private double Cambio { set; get; }
        private  int Cancelado { set; get; }
        private int Idusuario { set; get; }
        private int Hora { set; get; }
        private int Minutos { set; get; }
        public ticket()
        {
            Fechaventa = "";
        ...
        public ticket(string fechaV,double totalT,double Monto,double Vuelto,int cancelado,int id_usuario,int hora,int minutos)
        {
            Fechaventa = fechaV;
            Totalticket = totalT;
            Pago = Monto;
        ...
        public DataSet ConsultarTicket(string valor1, string valor2)
        {
            ...
            try
            {
                SqlCeCommand _cmd;
                DB.conectar();
                _cmd = new SqlCeCommand("select * from T_Ticket where FechaVenta BETWEEN '"+valor1+"' AND '"+valor2+"'", DB.conexion());
        ...
        public DataSet ConsultartodosTicket()
        {
            DataSet data = new DataSet();
            ...
            try
            {
                SqlCeCommand _cmd;
                DB.conectar();
                _cmd = new SqlCeCommand("select * from T_Ticket", DB.conexion());
                SqlCeDataAdapter res = new SqlCeDataAdapter();
                res.SelectCommand = _cmd;
                res.Fill(data);
                DB.deconectar();
        ...
        public int obtener_ultimo_ticket()
        {
            int id = 0;
            try
            {
                SqlCeCommand _cmd;
                DB.conectar();
                _cmd = new SqlCeCommand("Select Max(Ticket) from T_Ticket", DB.conexion());
        ...
        public int agregar_ticket()
        {
            ...
            try
            {
                SqlCeCommand _cmd;
                DB.conectar();
                _cmd = new SqlCeCommand("INSERT INTO T_TICKET(FechaVenta,TotalTicket,Pago,Cambio," +
                                                                "Cancelado,Idusuario,Hora,Minutos) " +
                    " VALUES('" + Fechaventa + "'," + Totalticket + "" +
                     "," + Pago + "," + Cambio + "," + Cancelado+ "," + Idusuario + ","+Hora+ ","+Minutos+ ")", DB.conexion());
               id =Convert.ToInt32( _cmd.ExecuteScalar());
                DB.deconectar();
            }
            catch (Exception e)
